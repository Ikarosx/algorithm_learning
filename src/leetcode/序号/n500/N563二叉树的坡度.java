...
原题地址：https://leetcode-cn.com/problems/binary-tree-tilt/
  
 *  
 * @author Ikarosx 
 * @date 2021/7/12 
 */ 
 public class N563二叉树的坡度 {
 	private int sum = 0;
	...
	// 返回的是该树之和
	private int findTiltImpl(TreeNode root) {
		if (root == null) {
			return 0;
		}
		int l = findTiltImpl(root.left);
		int r = findTiltImpl(root.right);
		sum += Math.abs(l - r);
		return l + +r + root.val;
	}
}